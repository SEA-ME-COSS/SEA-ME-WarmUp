...
void search() {
    ...
    if (index >= 0 && index < phonebook.size()) {
        ...
        std::cout << "Bookmark status: " << phonebook[index].isBookmarked << std::endl;

        std::string bookmark;
        if (phonebook[index].isBookmarked == true) {
            std::cout << "Would you like to except bookmark?(y/n)";
            std::cin >> check_bookmark;
        }
